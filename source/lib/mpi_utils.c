...
void MPI_setup(Simulation* sim) {
     ...
     MPI_Aint displacements[4] = {offsetof(Particle, position),
                                  offsetof(Particle, velocity),
                                  offsetof(Particle, acceleration),
                                  offsetof(Particle, mass)};
     MPI_Datatype types[4] = {type_vec3d, type_vec3d,
                              type_vec3d, MPI_DOUBLE};
...
void MPI_sync_particles(Simulation* sim){
    ...
    for (int pnum = 0; pnum < sim->MPI_pcount; pnum++) {
        ...
        MPI_Irecv(sim->recv_arrays[pnum].particles,
                  recv_sizes[pnum], 
                  sim->MPI_particle_type,
                  pnum,
                  0,
                  MPI_COMM_WORLD,
                  &requests[pnum]);
